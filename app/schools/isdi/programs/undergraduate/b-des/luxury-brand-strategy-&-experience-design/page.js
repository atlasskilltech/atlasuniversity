...
import {
  isdiBdesLuxuryHero,
  isdiBdesLuxuryBrochures,
  isdiBdesLuxuryStats,
  isdiBdesLuxuryProgram,
  isdiBdesLuxuryAdmissions,
  isdiBdesLuxuryEligibility,
  isdiBdesLuxuryWhy,
  isdiBdesLuxuryExecution,
  isdiBdesLuxuryProjects,
  isdiBdesLuxuryVoices,
  isdiBdesLuxuryBanner,
  isdiBdesLuxuryFaq,
  isdiAboutSearch,
} from '@/lib/schools/isdi/programsBdesLuxuryContent';

/*
 * ref luxury-brand-strategy-&-experience-design.php `$title` / `$description`, written
 ...
 */
const TITLE = 'Luxury Brand Strategy  – ATLAS ISDI';
const DESCRIPTION =
...
export const metadata = {
  ...
  alternates: {
    canonical:
      'https://atlasuniversity.edu.in/schools/isdi/programs/undergraduate/b-des/luxury-brand-strategy-&-experience-design',
  },
  openGraph: {
...
/**
 * /schools/isdi/programs/undergraduate/b-des/luxury-brand-strategy-&-experience-design —
 * the ISDI header's "Programs > Undergraduate Degree > B.Des > B.Des in Luxury Brand
 * Strategy", and the ninth of the twelve B.Des programme pages.
 *
 * The same twelve blocks in the same order as the other eight, so every component is
 * reused unchanged and only the data differs: four years whose last has **four**
 * semesters (electives in Year 1 only), 6 / 6 / 31 cards and 14 people in the tab panes,
 * 9 and 9 execution cards, four Student Voices cards and seven FAQ items. Its
 * "More questions?" block is the bare include and its stack is the `pro-bg-*` palette,
 * as on the Accessory page.
 *
 * `<main>` carries the ISDI body type (`body.isdi-page`: Poppins 16px/24, #333) and
 * must not clip, so the stack cards can stick.
 */
export default function ISDIBdesLuxuryPage() {
  return (
    <main id="isdi-bdes" className="font-poppins-isdi text-base text-[#333]">
      <Hero data={isdiBdesLuxuryHero} brochures={isdiBdesLuxuryBrochures} />
      <StatsBand data={isdiBdesLuxuryStats} />
      ...
      <section data-block="program" className="block overflow-hidden py-[50px]">
        <div className={CONTAINER}>
          <ProgramTabs data={isdiBdesLuxuryProgram} />
          <Curriculum data={isdiBdesLuxuryProgram.curriculum} brochures={isdiBdesLuxuryBrochures} />
        </div>
      </section>

      <Admissions data={isdiBdesLuxuryAdmissions} />
      <Eligibility data={isdiBdesLuxuryEligibility} />
      <WhyStack data={isdiBdesLuxuryWhy} />
      <Execution data={isdiBdesLuxuryExecution} />
      <Projects data={isdiBdesLuxuryProjects} />
      <Voices data={isdiBdesLuxuryVoices} />
      ...
      */}
      <ReadyToBuild
        data={isdiBdesLuxuryBanner}
        renderButton={(button) =>
          button.brochure ? (
            <BrochureButton
              key={button.label}
              brochure={isdiBdesLuxuryBrochures[button.brochure]}
              label={button.label}
