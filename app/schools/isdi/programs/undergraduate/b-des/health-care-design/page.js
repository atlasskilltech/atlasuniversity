...
import {
  isdiBdesHealthCareHero,
  isdiBdesHealthCareBrochures,
  isdiBdesHealthCareStats,
  isdiBdesHealthCareProgram,
  isdiBdesHealthCareAdmissions,
  isdiBdesHealthCareEligibility,
  isdiBdesHealthCareWhy,
  isdiBdesHealthCareExecution,
  isdiBdesHealthCareProjects,
  isdiBdesHealthCareVoices,
  isdiBdesHealthCareBanner,
  isdiBdesHealthCareFaq,
  isdiAboutSearch,
} from '@/lib/schools/isdi/programsBdesHealthCareContent';
 ...
 */
const TITLE = 'Health & Care Design Designing Innovations That Reach Patients';
const DESCRIPTION =
...
export const metadata = {
  ...
  alternates: {
    canonical:
      'https://atlasuniversity.edu.in/schools/isdi/programs/undergraduate/b-des/health-care-design',
  },
  openGraph: {
...
/**
 * /schools/isdi/programs/undergraduate/b-des/health-care-design — the ISDI header's
 * "Programs > Undergraduate Degree > B.Des > B.Des in Health & Care Design", and the
 * seventh of the twelve B.Des programme pages.
 *
 * The same twelve blocks in the same order as the other six, so every component is
 * reused unchanged and only the data differs: four years (Year 4 has four semesters,
 * electives in Year 1 only), nine cards per execution tab, four Student Voices cards and
 * seven FAQ items. Its "More questions?" block is the bare include and its stack is the
 * `pro-bg-*` palette, as on the Accessory page — whose sticky-stack heading it also
 * repeats verbatim ("Why Design Engineering at ISDI x Parsons"), an upstream copy-paste.
 *
 * `<main>` carries the ISDI body type (`body.isdi-page`: Poppins 16px/24, #333) and
 * must not clip, so the stack cards can stick.
 */
export default function ISDIBdesHealthCarePage() {
  return (
    <main id="isdi-bdes" className="font-poppins-isdi text-base text-[#333]">
      <Hero data={isdiBdesHealthCareHero} brochures={isdiBdesHealthCareBrochures} />
      <StatsBand data={isdiBdesHealthCareStats} />
      ...
      <section data-block="program" className="block overflow-hidden py-[50px]">
        <div className={CONTAINER}>
          <ProgramTabs data={isdiBdesHealthCareProgram} />
          <Curriculum data={isdiBdesHealthCareProgram.curriculum} brochures={isdiBdesHealthCareBrochures} />
        </div>
      </section>

      <Admissions data={isdiBdesHealthCareAdmissions} />
      <Eligibility data={isdiBdesHealthCareEligibility} />
      <WhyStack data={isdiBdesHealthCareWhy} />
      <Execution data={isdiBdesHealthCareExecution} />
      <Projects data={isdiBdesHealthCareProjects} />
      <Voices data={isdiBdesHealthCareVoices} />
      ...
      */}
      <ReadyToBuild
        data={isdiBdesHealthCareBanner}
        renderButton={(button) =>
          button.brochure ? (
            <BrochureButton
              key={button.label}
              brochure={isdiBdesHealthCareBrochures[button.brochure]}
              label={button.label}
      ...
      */}
      <div data-block="faq" className="-my-2.5 max-md:my-2.5">
        <FAQ data={isdiBdesHealthCareFaq} />
      </div>
    </main>
  );
}
