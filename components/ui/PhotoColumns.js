/**
 * `.grid-gallery-flex.sm.atlas > .grid-column.sm > .inst-img-wrap` — a clipped
 * `32px 0` frame of photographs laid out in columns, each captioned over a
 * scrim across its lower half.
 *
 * Two instances, measured identical in every property:
 *   /campus-atlas                      "A campus built to learn, live & thrive"
 *   /advantages/atlas-enterprenurship  "All the backing you need – In One Place"
 *
 * ── Measured cascade ──────────────────────────────────────────────────────
 *                            >=768px                     <=767px
 ...
 *
 * Several captions are empty (`<div class="inst-cont"></div>`), which paints
 * only the scrim — rendered as authored.
 */
export default function PhotoColumns({ columns }) {
  return (
    ...
    >
      {columns.map((column, index) => (
        ...
        >
          {column.map((item) => (
            /* ref .inst-img-wrap */
            <div key={item.image.src} className="relative max-md:w-1/2">
